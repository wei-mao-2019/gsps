...
class VAE2(nn.Module):
    ...
    def reparameterize(self, mu, logvar, nk=10):
        std = torch.exp(0.5 * logvar)
        # eps = torch.randn_like(std)
        eps = torch.randn([std.shape[0], nk, std.shape[1]], dtype=std.dtype, device=std.device)
    ...
    def forward(self, x, y, nk=10):
        l, n, h = x.shape
        mu, logvar = self.encode(x, y)
        z = self.reparameterize(mu, logvar, nk) if self.training else mu
        if self.training:
            x0 = x[:, :, None, :].repeat([1, 1, nk, 1]).reshape([l, n * nk, h])
            z0 = z.reshape([n * nk, self.nz]).clone().detach()
        return self.decode(x, z[:, 0]), mu, logvar, z0, self.decode(x0, z0)
...
def get_vae_model(cfg, traj_dim):
    ...
    if model_name == 'VAEv2':
        return VAE2(traj_dim, traj_dim, cfg.nz, cfg.t_pred, specs)
